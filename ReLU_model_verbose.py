...
def evaluate_model_extended(model, X_data, y_true, class_labels, model_name, dataset_name):
    """Evaluates a trained model on a given dataset and shows detailed metrics."""
    print(f"\n--- Evaluating {model_name} Model on {dataset_name} Data ---")
    try:
        loss, acc = model.evaluate(X_data, y_true, verbose=0)
        print(f"{model_name} - {dataset_name} Accuracy: {acc:.4f}")
        print(f"{model_name} - {dataset_name} Loss: {loss:.4f}")
    except Exception as e:
        print(f"Error during model.evaluate for {model_name} on {dataset_name}: {e}")
        return # Don't proceed if evaluation fails

    try:
        y_pred_probs = model.predict(X_data)
        y_pred_classes = np.argmax(y_pred_probs, axis=1)
    except Exception as e:
        print(f"Error during model.predict for {model_name} on {dataset_name}: {e}")
        return # Don't proceed if prediction fails

    if y_true.size == 0 or y_pred_classes.size == 0:
        print(f"Empty data for plots/report for {model_name} on {dataset_name}.")
        return

    # Confusion Matrix
    try:
        # Ensure class_labels covers all unique values in y_true and y_pred_classes for accurate plotting
        # If not, confusion_matrix might behave unexpectedly or error with explicit labels.
        # For simplicity, we assume class_labels corresponds to 0..N-1 classes.
        unique_true_labels = np.unique(y_true)
        unique_pred_labels = np.unique(y_pred_classes)
        
        # Check if all predicted/true labels are within the range of class_labels
        ...
        if max_label_val >= len(class_labels):
            print(f"Warning: Max label value ({max_label_val}) is outside the range of provided class_labels ({len(class_labels)}). Confusion matrix might be misleading.")
            # Fallback to default labels if necessary, or extend class_labels if appropriate
            # For now, we proceed with provided class_labels but this indicates a potential issue.

        cm_labels_indices = list(range(len(class_labels)))

        cm = confusion_matrix(y_true, y_pred_classes, labels=cm_labels_indices)
        plt.figure(figsize=(max(8, len(class_labels)), max(6, len(class_labels)*0.8)))
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
                    xticklabels=class_labels, yticklabels=class_labels)
        plt.xlabel('Predicted Label')
        plt.ylabel('Actual Label')
        plt.title(f'{model_name} Model - {dataset_name} Confusion Matrix')
        plt.tight_layout()
        plt.show()
    except Exception as e:
        print(f"Error generating {dataset_name} Confusion Matrix for {model_name}: {e}")

    # Classification Report
    print(f"\n--- {model_name} - {dataset_name} Classification Report ---")
    try:
        cm_report_labels_indices = list(range(len(class_labels)))
        # Ensure target_names matches the number of unique classes present or expected.
        print(classification_report(y_true, y_pred_classes, labels=cm_report_labels_indices,
                                      target_names=class_labels, zero_division=0))
    except Exception as e:
        print(f"Error generating {dataset_name} classification report for {model_name}: {e}")
...
async def main_async():
    ...
    try:
        (X_train, X_test,
         ...
         y_position_train, y_position_test) = train_test_split(
            ...
            test_size=0.2, random_state=RANDOM_STATE, stratify=all_y_grasp # Stratify by grasp
        )
        (X_train, X_val,
         y_grasp_train, y_grasp_val,
         y_position_train, y_position_val) = train_test_split(
            X_train, y_grasp_train, y_position_train,
            test_size=0.25, random_state=RANDOM_STATE, stratify=y_grasp_train # Stratify by grasp_train
    ...
    if not (len(X_train) > 0 and len(X_val) > 0 and len(X_test) > 0):
        print("Error: One or more data splits are empty. Cannot proceed with training.")
        return None, None, None, None
    ...
    if len(unique_pos_classes) > 1 : # compute_class_weight needs at least 2 classes
        try:
        ...
        print("Not enough classes in y_position_train to compute class weights. Proceeding without weights.")
        position_class_weights_dict = None


    # --- Model Creation ---
    # Ensure there's more than one class before defining labels and models
    num_grasp_classes = len(np.unique(all_y_grasp))
    num_position_classes = len(np.unique(all_y_position))

    if num_grasp_classes <= 1:
        print(f"Error: Only {num_grasp_classes} grasp class(es) found. Need at least 2 for classification. Exiting.")
        return None, None, None, None
    if num_position_classes <= 1:
        print(f"Error: Only {num_position_classes} position class(es) found. Need at least 2 for classification. Exiting.")
        return None, None, None, None
    ...
    print("\n--- Evaluating Grasp Model ---")
    evaluate_model_extended(grasp_model, X_train_scaled, y_grasp_train, grasp_labels, "Grasp", "Training")
    evaluate_model_extended(grasp_model, X_val_scaled, y_grasp_val, grasp_labels, "Grasp", "Validation")
    evaluate_model_extended(grasp_model, X_test_scaled, y_grasp_test, grasp_labels, "Grasp", "Test")

    print("\n--- Evaluating Position Model ---")
    evaluate_model_extended(position_model, X_train_scaled, y_position_train, position_labels, "Position", "Training")
    evaluate_model_extended(position_model, X_val_scaled, y_position_val, position_labels, "Position", "Validation")
    evaluate_model_extended(position_model, X_test_scaled, y_position_test, position_labels, "Position", "Test")
...
if __name__ == "__main__":
    ...
    if results:
        ...
        print("\nAsync pipeline did not complete successfully or exited early due to data issues.")